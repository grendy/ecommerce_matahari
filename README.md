# ecommerce_matahari
Crawling ecommerce matahari using scrapy and send json to kafka
##Understand the web structure
Must understand structure of the web as xpath and css selector
...
```bash
2 * * * * cd /root/crawler/ecommerce/matahari && python2.7 matahari.py > /dev/null 2>&1
1 * * * * kill -9 $(ps -ef|grep "matahari.py"|grep -v "grep"|awk '{print $2}')
```
